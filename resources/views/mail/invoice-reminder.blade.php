...
<body>
    <div class="wrapper">
        <div class="card">
            ...
            <h2>Reminder for invoice {{ $invoice->invoice_number }}</h2>
            <p>Hi {{ $invoice->client->name }}, this is a friendly reminder that this invoice from {{ $workspaceName }} is still open.</p>
            ...
            <div class="meta">
                ...
                <div class="meta-row">
                    ...
                    <span class="meta-value">{{ ucfirst($invoice->status) }}</span>
                </div>
            </div>

            @if($paymentUrl)
            <a href="{{ $paymentUrl }}" class="btn">Pay Invoice</a>
            @endif

            <p>The invoice PDF is attached again for your records.</p>
        </div>
